...
type DataTypeTreeController struct{}

func (this *DataTypeTreeController) GetMenuForDataType(w http.ResponseWriter, r *http.Request) {
	...
	if id == 0 {
		cmiNew := ContextMenuItem{"Create", "settings.dataType.new", "", "", false, nil, "node_create"}
		cmItems = append(cmItems, cmiNew)
	} else {
		cmiDel := ContextMenuItem{"Delete", "", "", "core/modules/settings/public/views/data-type/delete.html", true, nil, ""}
		cmiNew := ContextMenuItem{"Create", "settings.dataType.new", "", "", false, nil, "node_create"}
		cmItems = append(cmItems, cmiNew)
