...
angular.module("myApp").controller("TemplateTreeCtrlEdit", TemplateTreeCtrlEdit);

function TemplateTreeCtrl($scope, $stateParams, TemplateChildren, Template, sessionService, $interpolate, ngDialog) {
    $scope.rootNode = {
        "id": 1,
    ...
    $scope.expand_collapse = function(data) {
        if (!data.show) {
            ...
            if (data.nodes.length == 0) {
                ...
                data.nodes = TemplateChildren.query({
                    id: data.id
                }, function(node) {
                    //console.log(node)
                });
    ...
    var templateNodes = Template.query({
        'levels': '1'
    }, {}, function(node) {
        //console.log(node)
    });
    ...
    $scope.getEntityInfo = function(currentItem) {
        if (currentItem == undefined) {
            ...
            currentItem['entity'] = Template.get({
                id: currentItem.id
            }, function(data) {
...
function TemplateTreeCtrlEdit($scope, $stateParams, Template) {
    $scope.editorOptions = {
        lineWrapping: true,
        lineNumbers: true,
        readOnly: 'nocursor',
    ...
    if ($stateParams.id) {
        ...
        $scope.node = {
            "parent_template_node_id": parseInt($stateParams.parent)
        };
    }
    ...
    $scope.checkAll = function() {
        $scope.node.meta.allowed_templates_node_id = $scope.allTemplates.map(function(item) {
            return item.id;
        });
    };
