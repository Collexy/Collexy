...
angular.module("myApp").controller("MediaTypeTreeCtrlEdit", MediaTypeTreeCtrlEdit);
/**
 * @ngdoc controller
...
function MediaTypeTreeCtrl($scope, $stateParams, ContentTypeChildren, ContentType, sessionService, $interpolate, ngDialog) {
    ...
    $scope.expand_collapse = function(data) {
        if (!data.show) {
            ...
            if (data.nodes.length == 0) {
                ...
                data.nodes = ContentTypeChildren.query({
                    id: data.id
                }, function(node) {
                    //console.log(node)
                });
    ...
    var contentTypeNodes = ContentType.query({
        'type-id': '2',
        'levels': '1'
    }, {}, function(node) {
        //console.log(node)
    ...
    $scope.getEntityInfo = function(currentItem) {
        if (currentItem == undefined) {
            ...
            currentItem['entity'] = ContentType.get({
                id: currentItem.id
            }, function(data) {
    ...
    $scope.getMenu = function(node_type) {
        ...
        $scope.contextMenu = ContextMenu.query({}, {
            nodeType: node_type
        }, function(menu) {
            //alert("lol1")
        })
...
function MediaTypeTreeCtrlEdit($scope, $stateParams, ContentType, DataType) {
    ...
    if ($stateParams.id) {
        ...
        $scope.node = {
            "parent_content_type_node_id": parseInt($stateParams.parent)
        }
    } else {
        $scope.node = {}
    }
    if ($scope.stateParams.type) {
        if (typeof $scope.node.node !== 'undefined') {
            ...
            $scope.node["node"] = {
                node_type: parseInt($scope.stateParams.type)
            }
        }
    }
    // $scope.allTemplates = Node.query({'node-type': '3'},{},function(node){
    //   });
    // $scope.allContentTypes = ContentType.query({'type-id': '2'},{},function(node){
    //   });
    ...
    $scope.checkAll = function() {
        $scope.node.ct.meta.allowed_templates_node_id = $scope.allTemplates.map(function(item) {
            return item.id;
        });
    };
