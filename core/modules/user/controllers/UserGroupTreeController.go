...
type UserGroupTreeController struct{}

func (this *UserGroupTreeController) GetMenuForUserGroup(w http.ResponseWriter, r *http.Request) {
	...
	if id == 0 {
		cmiNew := ContextMenuItem{"Create", "user.userGroup.new", "", "", false, nil, "node_create"}
		cmItems = append(cmItems, cmiNew)
	} else {
		cmiDel := ContextMenuItem{"Delete", "", "", "core/modules/user/public/views/user-group/delete.html", true, nil, ""}
		cmiNew := ContextMenuItem{"Create", "user.userGroup.new", "", "", false, nil, "node_create"}
		cmItems = append(cmItems, cmiNew)
