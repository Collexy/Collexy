 ...
 */
function MediaTreeCtrl($scope, $stateParams, ContentChildren, Content, ContentType, sessionService, $interpolate, ngDialog) {
    ...
    $scope.getEntityInfo = function(currentItem) {
        ...
        if (currentItem.node_type != 5) {
            allowedContentTypes = [];
            currentItem['entity'] = Content.get({
                id: currentItem.id
            }, function(data) {
                var allowedContentTypes = [];
                ...
                for (var i = 0; i < data.content_type.meta.allowed_content_type_ids.length; i++) {
                    var ct = ContentType.get({
                        id: data.content_type.meta.allowed_content_type_ids[i]
                    }, function() {});
...
//   }
// }]);
function MediaTreeCtrlEdit($scope, $http, $stateParams, Content, Template, ContentType) {
    //$scope._ = _;
    var tabs = [];
